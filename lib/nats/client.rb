...
module NATS
  class Client
    ...
    def initialize(socket, options)
      ...
      @options = options
      @ssid, @subs = 1, {}
      @err_cb = NATS::Client.err_cb
      @reconnect_timer, @needed = nil, nil
      @reconnect_cb = NATS::Client.reconnect_cb
    ...
    def start_run_loop
      @threads << Thread.new do
        while !@closing do
          interval = (@timers.wait_interval || TICK_INTERVAL).abs
          begin
    ...
    def timeout(sid, timeout, opts={}, &callback)
      ...
      sub[:timeout].cancel if sub[:timeout]

      sub[:timeout] = after(timeout) do
    ...
    def request(subject, data=nil, opts={}, &cb)
      return unless subject
      inbox = NATS::Client.create_inbox
    ...
    def user_err_cb? # :nodoc:
      err_cb_overridden || NATS::Client.err_cb_overridden
    end
    ...
    def schedule_reconnect(wait=RECONNECT_TIME_WAIT) #:nodoc:
      ...
      @connected = false
      @reconnect_timer = @timers.every(wait) { attempt_reconnect }
    end
    ...
    def process_disconnect #:nodoc:
      ...
      @reconnect_timer.cancel if @reconnect_timer
      if (NATS::Client.client == self)
        NATS::Client.clear_client
        close if ((connected? || reconnecting?) and closing?)
      end
    ...
    def inspect #:nodoc:
      "<nats client v#{NATS::Client::VERSION}>"
    end
  end
end
